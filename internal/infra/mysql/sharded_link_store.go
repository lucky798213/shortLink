...
// ShardedLinkStore 是基于 MySQL 分片表的短链接存储实现。
type ShardedLinkStore struct {
	db       *gorm.DB
	strategy sharding.Strategy
}

// NewShardedLinkStore 创建分片短链接存储。
func NewShardedLinkStore(db *gorm.DB, shardCount int) *ShardedLinkStore {
...
func (r *ShardedLinkStore) FindByShortCode(ctx context.Context, shortCode string) (*shortlink.Link, error) {
...
func (r *ShardedLinkStore) DeleteByShortCode(ctx context.Context, shortCode string) (bool, error) {
	table, id, err := r.strategy.TableByShortCode(shortCode)
	if err != nil {
		return false, nil
	}
...
func (r *ShardedLinkStore) BatchCreate(ctx context.Context, rows []shortlink.CreateInput) error {
	//没有要插入的数据，直接返回成功
	if len(rows) == 0 {
		return nil
	}
	...
	for _, row := range rows {
		...
		//校验短码是否合法。
		if _, err := code.Decode(row.ShortCode); err != nil {
...
func (r *ShardedLinkStore) ScanActiveShortCodes(ctx context.Context, batchSize int, fn func([]string) error) error {
	if batchSize <= 0 {
		batchSize = 1000
	}
...
func (r *ShardedLinkStore) SoftDeleteExpired(ctx context.Context, now time.Time, batchSize int) ([]string, error) {
	if batchSize <= 0 {
		batchSize = 500
	}
...
var _ app.BatchLinkStore = (*ShardedLinkStore)(nil)
var _ app.MaintenanceStore = (*ShardedLinkStore)(nil)
